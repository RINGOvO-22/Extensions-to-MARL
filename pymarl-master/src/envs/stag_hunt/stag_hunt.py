...
class StagHunt(MultiAgentEnv):
    ...
    def step(self, actions):
        ...
        if not self.batch_mode:
            actions = np.expand_dims(np.asarray(actions.cpu(), dtype=int_type), axis=1)  # Move to CPU and then convert
        else:
            actions = np.asarray(actions.cpu(), dtype=int_type)  # Move to CPU and then convert
